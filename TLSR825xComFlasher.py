...
def sws_rd_addr(addr):
    return sws_encode_blk(
        bytearray([0x5A, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF, 0x80])
    )
...
def sws_wr_addr(addr, data):
    return sws_encode_blk(
        bytearray([0x5A, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF, 0x00])
        + bytearray(data)
    ) + sws_encode_blk([0xFF])
...
def rd_sws_fifo_wr_usbcom(serialPort, addr, data):
    rd_sws_wr_addr_usbcom(
        serialPort, 0x00B3, bytearray([0x80])
    )  # [0xb3]=0x80 ext.SWS into fifo mode
    rd_sws_wr_addr_usbcom(
        serialPort, addr, data
    )  # send all data to one register (no increment address - fifo mode)
    rd_sws_wr_addr_usbcom(
        serialPort, 0x00B3, bytearray([0x00])
...
def set_sws_speed(serialPort, clk):
    ...
    if len(read) != byteSent:
        if (
            serialPort.baudrate > USBCOMPORT_BAD_BAUD_RATE
            and byteSent > 64
            and len(read) >= 64
            and len(read) < byteSent
        ):
            print("\n\r!!!!!!!!!!!!!!!!!!!BAD USB-UART Chip!!!!!!!!!!!!!!!!!!!")
            print("UART Output:")
    ...
    if x != None and x[0] == swsdiv:
        ...
        if debug:
            print(
                "Debug: UART-SWS %d baud. SW-CLK ~%.1f MHz"
                % (
                    int(serialPort.baudrate / 10),
                    serialPort.baudrate * swsdiv / 2000000,
                )
            )
...
def set_sws_auto_speed(serialPort):
    ...
    if debug:
        ...
        print(
            "Debug: default swdiv for 24 MHz = %d (0x%02x)" % (swsdiv_def, swsdiv_def)
        )
...
def FlashReadBlock(serialPort, stream, offset=0, size=0x80000):
    ...
    while size > 0:
        ...
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0B3, bytearray([0x80])
        )  # [0xb3]=0x80 ext.SWS into fifo mode
        ...
        rd_sws_wr_addr_usbcom(
            ...
            0x0C,
            bytearray(
                [0x03, (offset >> 16) & 0xFFFF, (offset >> 8) & 0xFF, offset & 0xFF, 0]
            ),
        )
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0D, bytearray([0x0A])
        )  # [0x0d]=0x0a SPI set auto read mode & cns low
        ...
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0B3, bytearray([0x00])
        )  # [0xb3]=0x00 ext.SWS into normal(ram) mode
...
def FlashReady(serialPort, count=33):
    for _ in range(count):
        rd_sws_wr_addr_usbcom(serialPort, 0x0D, bytearray([0x00]))  # SPI set cns low
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0C, bytearray([0x05])
        )  # Flash cmd rd status
...
def FlashWriteAddr(serialPort, addr, data):
    ...
    rd_sws_fifo_wr_usbcom(
        serialPort,
        0x0C,
        bytearray([0x02, (addr >> 16) & 0xFFFF, (addr >> 8) & 0xFF, addr & 0xFF])
        + bytearray(data),
    )
    rd_sws_wr_addr_usbcom(serialPort, 0x0D, bytearray([0x01]))  # cns high
...
def FlashEraseSectors(serialPort, offset=0, size=1):
    ...
    while size > 0:
        ...
        rd_sws_wr_addr_usbcom(serialPort, 0x0D, bytearray([0x00]))  # cns low
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0C, bytearray([0x20])
        )  # Flash cmd erase sector
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0C, bytearray([(offset >> 16) & 0xFFFF])
        )  # Faddr hi
        rd_sws_wr_addr_usbcom(
            serialPort, 0x0C, bytearray([(offset >> 8) & 0xFF])
        )  # Faddr mi
...
def main():
    ...
    parser.add_argument(
        "-b",
        "--baud",
        help="UART Baud Rate (default: "
        + str(COMPORT_DEF_BAUD_RATE)
        + ", min: "
        + str(COMPORT_MIN_BAUD_RATE)
        + ")",
        type=arg_auto_int,
        default=COMPORT_DEF_BAUD_RATE,
    )
    parser.add_argument(
        "-r", "--run", help="CPU Run (post main processing)", action="store_true"
    )
    ...
    parser_read_flash = subparsers.add_parser("rf", help="Read Flash to binary file")
    ...
    parser_burn_flash = subparsers.add_parser(
        "wf", help="Write file to Flash with sectors erases"
    )
    ...
    parser_erase_sec_flash.add_argument(
        "address", help="Start address", type=arg_auto_int
    )
    parser_erase_sec_flash.add_argument(
        "size", help="Size of region", type=arg_auto_int
    )

    parser_erase_all_flash = subparsers.add_parser("ea", help="Erase All Flash")
